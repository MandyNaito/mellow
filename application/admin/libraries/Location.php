...
class Location {
	...
	{	
		...
		$this->ci->load->config('location');		
		$this->apikey = $this->ci->config->item('apikey');
		
		log_message('debug', "Location Class Initialized");
	}
	...
	public function reverseAddress($address) {
		...
		if ($use_curl) {
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://maps.googleapis.com/maps/api/geocode/json?address=" . urlencode($address) . "&sensor=true&key=".$this->apikey);
			...
			$str = @file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?address=" . urlencode($address) . "&sensor=true&key=".$this->apikey);
			$data = json_decode($str, true);
			if (isset($data["results"]) && is_array($data["results"])) {
				$latLong = $data["results"][0]["geometry"]["location"];
				$meta = array(
	...
	public function getWeather($lac, $cid) {
		...
		if (is_array($latLng)) {
			...
			$use_curl = false;
			$url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" . $lat . "," . $lng . "&sensor=true&key=".$this->apikey;
			$str = "";
			if ($use_curl) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, $url);
			...
			if (isset($data["results"]) && is_array($data["results"])) {
				...
				$url = "https://www.google.com/ig/api?weather=".str_replace("\"", "", $city);
				return $this->getContent("https://www.google.com/ig/api?weather=Depok,ID");
			} 
			else
				return "false";
		} 
	...
	public function getAddress($lat, $lng) {
		...
		if ($use_curl) {
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://maps.googleapis.com/maps/api/geocode/json?latlng=" . $lat . "," . $lng . "&sensor=true&key=".$this->apikey);
			curl_setopt($ch, CURLOPT_HEADER, true);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_BINARYTRANSFER, 1);
			curl_setopt($ch, CURLOPT_HTTPHEADER, Array (
			...
			$str = @file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?latlng=" . $lat . "," . $lng . "&sensor=true&key=".$this->apikey);
			$data = json_decode($str, true);
			if (isset($data["results"]) && is_array($data["results"])) {
				$street = $data["results"][0]["formatted_address"];
				$ac = $data["results"][0]["address_components"];
	...
	public function getLatLong($lac, $cid) {
		...
		if (isset ($lac) && isset ($cid)) {
			...
			if ($use_curl) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "https://www.google.com/glm/mmap");
				curl_setopt($ch, CURLOPT_HEADER, true);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($ch, CURLOPT_BINARYTRANSFER, 1);
				curl_setopt($ch, CURLOPT_HTTPHEADER, Array (
				...
				$str = file_get_contents("https://www.google.com/glm/mmap", FALSE, $xcontext);
			}
	...
	// Reff: https://code.google.com/intl/id-ID/apis/maps/documentation/directions/
	// https://localhost/rest/index.php/api/location/direction/<origin>/<destination>
	// e.g: https://localhost/rest/index.php/api/location/direction/Sawangan-Depok/Senayan-Jakarta
	public function getDirection($origin, $destination, $travelMode) {
		$use_curl = false;
		if ($use_curl) {
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://maps.googleapis.com/maps/api/directions/json?origin=".$origin."&destination=".$destination."&mode=".$travelMode."&sensor=false");
			curl_setopt($ch, CURLOPT_HEADER, true);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_BINARYTRANSFER, 1);
			curl_setopt($ch, CURLOPT_HTTPHEADER, Array (
			...
			$str = @file_get_contents("https://maps.googleapis.com/maps/api/directions/json?origin=".$origin."&destination=".$destination."&mode=".$travelMode."&sensor=false");
			$data = json_decode($str, true);
			if (isset($data["status"]) && $data["status"] == "OK") {
				$meta = array(
					"status"=>200,
